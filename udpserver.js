...
//add your blynk information here
...
serverSocket.on('message', function(msg, rinfo){
  ...
  if (cmd === 'iam') {
    ...
    var data = JSON.parse(json['data']);
    if (model === 'sensor_ht') {
      temperature = (data['temperature'] != null) ? data['temperature'] / 100.0 : temperature;
      humidity = (data['humidity'] != null) ? data['humidity'] / 100.0 : humidity;
	  tmp = 'http://blynk-cloud.com/'+AUTH+'/update/'+v_temp+'?value='+temperature;
	  request(tmp, function (error, response, body) {});
	  tmp = 'http://blynk-cloud.com/'+AUTH+'/update/'+v_hum+'?value='+humidity;
	  request(tmp, function (error, response, body) {});
	  ...
	  tmp = 'http://blynk-cloud.com/'+AUTH+'/update/'+v_door+'?value='+data['status'];
      request(tmp, function (error, response, body) {});
    ...
    if (model === 'sensor_ht') {
      ...
      console.log("Step 6. Got motion sensor:%s's report: move %s", json['sid'], (data['status'] === 'motion') ? 'detected' : 'not detected');
    	...
    	door = (data['status'] != null) ? data['status'] : door;
    	
	  tmp = 'http://blynk-cloud.com/'+AUTH+'/update/'+v_door+'?value='+door;
